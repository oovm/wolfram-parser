    ...
    fn parse<'i>(
        ...
        mod rules {
                ...
                pub fn skip(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    if state.atomicity() == ::pest::Atomicity::NonAtomic {
                        state.sequence(|state| {
                            state.repeat(|state| super::visible::WHITESPACE(state)).and_then(|state| {
                                state.repeat(|state| {
                                    state.sequence(|state| {
            ...
            pub mod visible {
                                ...
                                state.sequence(|state| {
                                    state.optional(|state| {
                                        self::statement(state).and_then(|state| {
                                            state.repeat(|state| {
                                                state.sequence(|state| {
                                                    super::hidden::skip(state).and_then(|state| self::statement(state))
                                                })
                                            })
                                        })
                                    })
                        ...
                        state.rule(Rule::apply, |state| {
                            state.sequence(|state| {
                                state
                                    .match_string("[")
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| state.optional(|state| self::apply_kv(state)))
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| {
                                        state.sequence(|state| {
                                            state.optional(|state| {
                ...
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn function_module(
                    state: Box<::pest::ParserState<Rule>>,
                ) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::function_module, |state| {
                        state.sequence(|state| {
                            self::Dot(state).and_then(|state| super::hidden::skip(state)).and_then(|state| {
                                state.sequence(|state| {
                                    state.optional(|state| {
                                        state
                                            .sequence(|state| {
                ...
                pub fn term(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::term, |state| {
                            state.sequence(|state| {
                                state.repeat(|state| self::Prefix(state)).and_then(|state| self::node(state)).and_then(
                                    |state| {
                                        state.repeat(|state| {
                                            self::Suffix(state).or_else(|state| {
                                                state.sequence(|state| {
                                                    state
                                                        .repeat(|state| {
                                                            state.sequence(|state| {
                ...
                pub fn node(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::node, |state| {
                        state
                            ...
                            .or_else(|state| self::apply_call(state))
                            .or_else(|state| self::derivative(state))
                            .or_else(|state| self::space_call(state))
                ...
                pub fn apply_call(
                    ...
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::apply_call, |state| {
                            state.sequence(|state| {
                                self::Symbol(state)
                                    .and_then(|state| {
                                        state.sequence(|state| {
                ...
                pub fn derivative(
                    ...
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::derivative, |state| {
                            state.sequence(|state| {
                                self::Symbol(state)
                                    .and_then(|state| self::Derivative(state))
                                    .and_then(|state| {
                                        state.repeat(|state| {
                                            state.sequence(|state| {
                                                state
                ...
                pub fn space_call(
                    ...
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::space_call, |state| {
                            state.sequence(|state| {
                    ...
                    state.atomic(::pest::Atomicity::NonAtomic, |state| {
                        state.rule(Rule::key_value, |state| {
                            state.sequence(|state| {
                                self::data(state)
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| self::RuleDelayed(state).or_else(|state| self::Rule(state)))
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| self::expr(state))
                            })
                        })
                ...
                pub fn index(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::index, |state| {
                        state
                            .sequence(|state| {
                                state
                                    .optional(|state| self::Start(state))
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| state.match_string(";;"))
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| state.optional(|state| self::End(state)))
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| state.match_string(";;"))
                                    .and_then(|state| super::hidden::skip(state))
                                    .and_then(|state| state.optional(|state| self::Step(state)))
                            })
                            .or_else(|state| {
                                state.sequence(|state| {
                                    state
                        ...
                        state.rule(Rule::Number, |state| {
                            state.sequence(|state| {
                                self::BaseForm(state)
                                    ...
                                    .and_then(|state| state.optional(|state| self::ExDigits(state)))
                            })
                        })
                    })
                }
                ...
                pub fn Decimal(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::Decimal, |state| {
                            state.sequence(|state| self::Dot(state).and_then(|state| self::Integer(state))).or_else(|state| {
                                state.sequence(|state| {
                                    self::Integer(state)
                                        .and_then(|state| self::Dot(state))
                ...
                pub fn Integer(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::Integer, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                self::ASCII_DIGIT(state).and_then(|state| state.repeat(|state| self::ASCII_DIGIT(state)))
                            })
                        })
                    })
                }
                ...
                pub fn BaseForm(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::BaseForm, |state| {
                            state.sequence(|state| {
                                self::Integer(state)
                                    .and_then(|state| self::Base(state))
                                    .and_then(|state| self::ASCII_ALPHA(state).or_else(|state| self::ASCII_DIGIT(state)))
                                    .and_then(|state| {
                                        state.repeat(|state| self::ASCII_ALPHA(state).or_else(|state| self::ASCII_DIGIT(state)))
                                    })
                            })
                        })
                    })
                ...
                pub fn ExDigits(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::ExDigits, |state| {
                            state.sequence(|state| {
                                self::Accuracy(state)
                                    .or_else(|state| self::Precision(state))
                                    .and_then(|state| self::Decimal(state).or_else(|state| self::Integer(state)))
                            })
                        })
                    })
                }
                ...
                pub fn Accuracy(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Accuracy, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("``")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn Precision(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Precision, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("`")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                                ...
                                self::Quote(state)
                                    .and_then(|state| {
                                        state.repeat(|state| {
                                            state.sequence(|state| {
                                                state
                                                    .lookahead(false, |state| self::Quote(state))
                ...
                pub fn Symbol(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::Symbol, |state| {
                            state.sequence(|state| {
                                self::SYMBOL(state).and_then(|state| {
                                    state.repeat(|state| {
                                        state.sequence(|state| state.match_string("`").and_then(|state| self::SYMBOL(state)))
                                    })
                                })
                            })
                        })
                ...
                pub fn SYMBOL(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::SYMBOL, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state
                                .sequence(|state| {
                                    self::XID_START(state).and_then(|state| {
                                        state.repeat(|state| self::XID_START(state).or_else(|state| self::ASCII_DIGIT(state)))
                                    })
                                })
                                .or_else(|state| {
                                    state.sequence(|state| {
                                        state.match_string("$").and_then(|state| {
                                            state.repeat(|state| {
                                                self::XID_START(state).or_else(|state| self::ASCII_DIGIT(state))
                                            })
                                        })
                                    })
                                })
                ...
                pub fn Pattern(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::Pattern, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                state
                                    .repeat(|state| self::XID_START(state))
                                    .and_then(|state| self::Underline(state))
                                    .and_then(|state| state.repeat(|state| self::Underline(state)))
                            })
                        })
                    })
                }
                ...
                pub fn Slot(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.atomic(::pest::Atomicity::CompoundAtomic, |state| {
                        state.rule(Rule::Slot, |state| {
                            state
                                .sequence(|state| {
                                    state.match_string("##").and_then(|state| state.optional(|state| self::Positive(state)))
                                })
                                .or_else(|state| {
                                    state.sequence(|state| {
                                        state.match_string("#").and_then(|state| {
                                            state.optional(|state| self::SYMBOL(state).or_else(|state| self::Integer(state)))
                                        })
                                    })
                                })
                        })
                ...
                pub fn Positive(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::Positive, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                    ...
                    state.atomic(::pest::Atomicity::Atomic, |state| {
                        state.sequence(|state| {
                            state
                                ...
                                .and_then(|state| {
                                    state.repeat(|state| {
                                        self::COMMENT(state).or_else(|state| {
                                            state.sequence(|state| {
                                                state
                    ...
                    state.rule(Rule::Prefix, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            self::Bang(state)
                                ...
                                .or_else(|state| self::Plus(state))
                                .or_else(|state| self::Minus(state))
                                .or_else(|state| self::Star(state))
                        })
                    })
                }
                #[inline]
                ...
                pub fn Suffix(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::Suffix, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            self::Derivative(state)
                                .or_else(|state| self::DoubleBang(state))
                                .or_else(|state| self::Bang(state))
                                .or_else(|state| self::Question(state))
                        })
                    })
                }
                #[inline]
                ...
                pub fn Infix(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    self::Set(state)
                        .or_else(|state| self::Plus(state))
                        .or_else(|state| self::Minus(state))
                        .or_else(|state| self::Multiply(state))
                        ...
                        .or_else(|state| self::Power(state))
                        .or_else(|state| self::Grater(state))
                        .or_else(|state| self::GraterEqual(state))
                        .or_else(|state| self::Equal(state))
                        .or_else(|state| self::Dot(state))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Derivative(
                    ...
                    state.rule(Rule::Derivative, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.sequence(|state| {
                                state.match_string("′").and_then(|state| state.repeat(|state| state.match_string("′")))
                            })
                        })
                    })
                }
                ...
                pub fn Separate(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Separate, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string(";;")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn Semicolon(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Semicolon, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string(";")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Underline(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Underline, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("_")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn Load(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::Load, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            state.match_string("<<<").or_else(|state| state.match_string("⋘"))
                        })
                    })
                ...
                pub fn Pipeline(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Pipeline, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("|>")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn RuleDelayed(
                    ...
                    state.rule(Rule::RuleDelayed, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| state.match_string(":>"))
                    })
                }
                #[inline]
                ...
                pub fn Multiply(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state.rule(Rule::Multiply, |state| {
                        state.atomic(::pest::Atomicity::Atomic, |state| {
                            self::Star(state).or_else(|state| state.match_string("×"))
                        })
                    })
                }
                #[inline]
                ...
                pub fn CenterDot(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::CenterDot, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("⋅")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn Kronecker(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Kronecker, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("⊗")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Quotient(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Quotient, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("//")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Remainder(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Remainder, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("÷")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Increase(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Increase, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("++")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn Decrease(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Decrease, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("--")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                ...
                pub fn Destruct(state: Box<::pest::ParserState<Rule>>) -> ::pest::ParseResult<Box<::pest::ParserState<Rule>>> {
                    state
                        .rule(Rule::Destruct, |state| state.atomic(::pest::Atomicity::Atomic, |state| state.match_string("~=")))
                }
                #[inline]
                #[allow(non_snake_case, unused_variables)]
                pub fn DoubleBang(
